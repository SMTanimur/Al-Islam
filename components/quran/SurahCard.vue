...
<template>
  <NuxtLink
    ...
    class="group w-full cursor-pointer rounded-lg border border-gray-300/70 p-3 hover:border-rose-600 hover:shadow-surah-card dark:border-gray-700 dark:hover:border-rose-600 dark:hover:shadow-surah-card-dark"
  >
    <div class="mb-3 flex items-center justify-between">
      <!-- Number -->
      <div
        class="flex h-10 w-10 items-center justify-center rounded-full bg-gray-200/50 text-xs font-medium text-yami group-hover:bg-rose-600 group-hover:text-white dark:bg-slate-700 dark:text-slate-300"
      >
        {{ surah.nomor }}
      </div>
      ...
      <UTooltip :text="surah.isFavorite ? 'Hapus dari favorit' : 'Tambah ke favorit'">
        <Icon
          ...
          :class="surah.isFavorite ? 'text-rose-600' : 'text-slate-500 dark:text-slate-300'"
          class="z-10 text-xl"
